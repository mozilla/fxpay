    ...
    utils.getAppSelf(function(error, appSelf) {
      ...
      if (!platformHasAddReceipt && !settings.localStorage) {
        return callback(errors.PayPlatformUnavailable(
                          'no way to store receipts on this platform'));
      }

      if (appSelf && settings.window.location.href.indexOf('app://') === 0 &&
          !appSelf.manifest.origin) {
        return callback(errors.InvalidApp(
                            'packaged app did not define an origin so ' +
