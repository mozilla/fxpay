define([
  ...
  describe('jwt.decode()', function() {
    ...
    it('should decode JWTs', function(done) {
      ...
      jwt.decode(encJwt, function(err, data) {
    ...
    it('should error on missing segments', function(done) {
      jwt.decode('one.two', function(err) {
    ...
    it('should error on invalid binary data', function(done) {
      jwt.decode('<algo>.this{}IS not*base64 encoded.<sig>',
        function(err) {
          assert.instanceOf(err, errors.InvalidJwt);
          done();
        }
    ...
    it('should error on non JSON data within JWT', function(done) {
      ...
      jwt.decode(encJwt, function(err) {
        assert.instanceOf(err, errors.InvalidJwt);
        done();
      });
    });
  ...
  describe('jwt.getPayUrl()', function() {

    it('should pass through parse errors', function(done) {
      var encJwt = '<algo>.' + btoa('(not / valid JSON}') + '.<sig>';
      jwt.getPayUrl(encJwt, function(err) {
        assert.instanceOf(err, errors.InvalidJwt);
        done();
      });
    });
    ...
    it('should return a JWT formatted URL', function(done) {
      ...
      jwt.getPayUrl(encJwt, function(err, payUrl) {
