(function(exports) {
  ...
  var settings = {
    ...
    // Public settings.
    //
    // This is your Marketplace app ID.
    appId: null,
    ...
    callbacks: {
      ...
      onrestore: function(error, info) {
        if (error) {
          console.error('error while restoring product:', info.productId,
                        'message:', error);
        } else {
  ...
  exports.init = function _init(opt) {
    ...
    if (opt.onrestore) {
      settings.callbacks.onrestore = opt.onrestore;
    }

    var validOptions = ['appId', 'onerror', 'oninit', 'onrestore'];
    for (var k in opt) {
    ...
    if (!settings.appId) {
      settings.log.error('appId cannot be empty');
      return settings.callbacks.onerror('INCORRECT_USAGE');
    }
    ...
    appRequest.onsuccess = function() {
      ...
      if (settings.appSelf.receipts) {
        for (var i = 0; i < settings.appSelf.receipts.length; i++) {
          receipt = settings.appSelf.receipts[i];
          settings.log.info('Installed receipt: ' + receipt);
          numReceipts++;
          exports.verifyReceipt(receipt, settings.callbacks.onrestore);
        }
      }
  ...
  function verifyReceiptStoreData(data, onVerify) {
    ...
    if (!settings.allowAnyAppReceipt) {
      // Make sure the receipt belongs only to this app.
      if (productInfo.appId !== settings.appId.toString()) {
        settings.log.error('Receipt was for app', productInfo.appId,
                           'but only app', settings.appId, 'is allowed');
        return onVerify('INVALID_RECEIPT', productInfo);
      }
    }
