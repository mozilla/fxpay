(function() {
  ...
  exports.validateAppReceipt = function validateAppReceipt(callback) {
    ...
    getAppSelf(function(err, appSelf) {
      if (err) {
        return callback(err, defaultProductInfo);
      }
      if (!appSelf) {
      ...
      receipts.all().forEach(function(receipt) {
      ...
      if (allAppReceipts.length === 0) {
        ...
        receipt = allAppReceipts[0];
        settings.log.info('Installed receipt: ' + receipt);
        return receipts.validateAppReceipt(receipt,
                                           function(error, productInfo) {
          settings.log.info('got verification result for', productInfo);
          callback(error, productInfo);
        });
      } else {
        // TODO: support multiple app stores? This is an unlikely case where
        // multiple app receipts are installed.
        settings.log.error(
  ...
  function getAppSelf(callback) {
    ...
    // TODO: return immediately if this function has already run.
    ...
    appRequest.onsuccess = function() {
      ...
      // we're not running as an app, this could be NULL.
      var appSelf = this.result;
      storeAppSelf(appSelf);
      settings.log.info('got appSelf from mozApps.getSelf()');
      callback(null, appSelf);
    };
